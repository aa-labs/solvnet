type LoanRequest = {
  ...
  type Match = {
    userLeaseRequest: LoanRequest;  // changed from userLoanRequest
  ...
  function findOptimalMatches(userLeaseRequests: LoanRequest[], solverRequests: LoanRequest[]): Match[] {
    ...
    // Sort userLeases by amount (largest first)
    ...
    for (const userLeaseRequest of sortedUserLeases) {
      ...
      if (possibleMatches.length > 0) {
        ...
        // Remove used solver from available pool
  ...
  // Find optimal matches for all requests
